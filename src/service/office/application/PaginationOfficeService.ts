...
export class PaginationVehicleService {
  ...
  onFilter(filter: Partial<PaginateOfficeOpts>) {
    this._paginate(filter);
    this.setFilter(filter);
  }

  private async _paginate(newFilter: Partial<PaginateOfficeOpts>) {
    try {
      const {
        paginatedOffices: { paginationOptions },
      } = getOfficeState();
  ...
  private setFilter(filter: Partial<PaginateOfficeOpts>) {
    changeState(({ paginatedOffices }) => {
      if (filter) {
        paginatedOffices.paginationOptions = {
          ...paginatedOffices.paginationOptions,
